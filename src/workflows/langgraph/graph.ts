...
import { BaseMessage, HumanMessage, AIMessage } from './corePolyfill';
...
export class LangGraph {
    ...
    public async execute(
        ...
        try {
            ...
            while (currentNodeId && steps < maxSteps) {
                ...
                // Execute node
                logger.info(`Executing node: ${currentNode.name} (${currentNodeId})`);
                ...
                try {
                    ...
                    logger.error(`Error executing node '${currentNode.name}':`, error);
            ...
            if (steps >= maxSteps) {
                logger.warn(`Execution reached max steps (${maxSteps})`);
            }
     ...
     * Get the next node
     */
    private async getNextNode(currentNodeId: string, state: GraphState): Promise<string> {
        // Get outgoing edges
    ...
    public static createAgentNode(id: string, name: string, agent: Agent): GraphNode {
        return {
            ...
            execute: async (state: GraphState): Promise<GraphState> => {
                ...
                state.messages.push(new AIMessage(result));
     ...
     */
    public static createToolNode(id: string, name: string, tool?: Tool | StructuredTool): GraphNode {
        return {
            id,
            type: 'tool',
            name,
            ...
            execute: async (state: GraphState): Promise<GraphState> => {
                ...
                // Execute the tool
                const result = await (tool as any).invoke(input);
     ...
     */
    public static createConditionalNode(
        id: string,
        name: string,
