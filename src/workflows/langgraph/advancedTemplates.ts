...
import { createMemoryRetrievalTool, createMemorySaveTool, createDocumentRetrievalTool } from './vectorStores';

// Custom tool implementations
class DocumentRetrievalStructuredTool extends StructuredTool {
    schema = z.object({
        input: z.string().optional().describe('The query to search for')
    }).transform((obj: { input?: string }) => obj.input || '');

    name: string;
    description: string;
    private retrievalTool: Tool | ITool;

    constructor(retrievalTool: Tool | ITool) {
        super();
...
class SearchStructuredTool extends StructuredTool {
    ...
    name: string;
    description: string;
    private searchTool: Tool | ITool;

    constructor(searchTool: Tool | ITool) {
        super();
...
export function createRAGWorkflow(
    ...
    description: string,
    agent: Agent,
    retrievalTool: StructuredTool | ITool
): GraphDefinition {
    // Create nodes
    const inputNode = LangGraph.createInputNode('input', 'Input');

    // Use the provided retrieval tool or create a document retrieval tool
    let documentRetrievalTool: Tool;

    if (retrievalTool) {
        documentRetrievalTool = new DocumentRetrievalStructuredTool(retrievalTool as Tool);
    } else {
        documentRetrievalTool = (createDocumentRetrievalTool() as unknown) as Tool;
    }

    const retrievalNode = LangGraph.createToolNode(
        'retrieval',
        'Document Retrieval',
        documentRetrievalTool
    );

    const contextFormattingNode: GraphNode = {
        ...
        execute: async (state): Promise<any> => {
            ...
            if (Array.isArray(retrievalResults)) {
                formattedContext = retrievalResults.map((doc, index) =>
...
export function createCollaborativeWorkflow(
    ...
    const specialistNodes: GraphNode[] = specialistAgents.map(specialistAgent => {
        return {
...
export function createMemoryEnhancedAgentWorkflow(
    ...
    const memoryRetrievalTool = createMemoryRetrievalTool();

    const memoryRetrievalNode: GraphNode = {
        id: 'memory-retrieval',
        type: 'tool',
        name: 'Memory Retrieval',
        tool: memoryRetrievalTool as Tool,
        execute: async (state): Promise<any> => {
            // Get the input message
            const lastMessage = state.messages.length > 0
                ? state.messages[state.messages.length - 1]
            ...
            logger.info('Retrieving memories for:', contentAsString);
    ...
    const memorySaveTool = createMemorySaveTool();

    const memorySaveNode: GraphNode = {
        id: 'memory-save',
        type: 'tool',
        name: 'Memory Save',
        tool: memorySaveTool as Tool,
        execute: async (state): Promise<any> => {
            ...
            logger.info('Saving memory from conversation:');
            logger.info('- User query:', contentAsString);
...
export function createResearchWorkflow(
    ...
    const informationGatheringNode: GraphNode = {
        ...
        execute: async (state): Promise<any> => {
            ...
            // Perform searches
            const searchResults: { query: any; result: string }[] = [];
    ...
    const informationAnalysisNode: GraphNode = {
        ...
        execute: async (state): Promise<any> => {
            ...
            const formattedResults = searchResults.map((result: { query: string; result: string }) =>
