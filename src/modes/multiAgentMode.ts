 ...
 */
export class MultiAgentMode extends BaseOperationMode {
...
`
    ...
    async processMessage(
        ...
        try {
            logger.info(`Processing message in Multi-Agent mode: ${message}`);
            ...
            logger.error('Error processing message in Multi-Agent mode:', error);
...
`;
        ...
        const planResponse = await this.supervisorAgent.generate(
            supervisorPrompt,
            this.getLLMParams(this.supervisorAgent.getDefaultLLMParams()),
            token
        );
...
`;
                ...
                const agentResponse = await agentInstance.agent.generate(
                    agentPrompt,
                    this.getLLMParams(agentInstance.agent.getDefaultLLMParams()),
                    token
                );
...
`;
    ...
    private stopMultiAgentSystem(reason: string): void {
        ...
        // Log reason
        logger.info(`Multi-Agent system stopped: ${reason}`);
    }
     ...
     */
    async getSystemPrompt(_agent: Agent, _contextSource: ContextSource): Promise<string> {
        return `
