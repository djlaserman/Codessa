 ...
 */
export class CodeParrotProvider extends BaseLLMProvider {
    readonly providerId = 'codeparrot';
    readonly displayName = 'CodeParrot';
    readonly description = 'Lightweight code model, good for fine-tuning';
    readonly website = 'https://huggingface.co/codeparrot';
    ...
    readonly defaultModel = 'codeparrot/codeparrot-small';
    ...
    constructor(context?: vscode.ExtensionContext) {
        ...
        vscode.workspace.onDidChangeConfiguration(e => {
            if (e.affectsConfiguration('codessa.llm.providers')) {
                logger.info("CodeParrot configuration changed, re-initializing client.");
    ...
    private initializeClient() {
        ...
        if (!apiKey) {
            logger.warn('API key not set for CodeParrot provider.');
            this.client = null;
            return;
        }
        ...
        try {
            ...
            logger.info('CodeParrot client initialized successfully.');
        } catch (error) {
            logger.error('Failed to initialize CodeParrot client:', error);
            this.client = null;
        }
    }
    ...
    async generate(
        ...
        try {
            ...
            logger.debug(`Sending request to CodeParrot model ${modelId}`);
            ...
            if (cancellationToken) {
                if (typeof AbortController !== 'undefined') {
                    ...
                    cancellationToken.onCancellationRequested(() => {
                        logger.info("CodeParrot request cancelled by user");
            ...
            logger.error('CodeParrot generate error:', error);
    ...
    public async testConnection(modelId: string): Promise<{success: boolean, message: string}> {
        ...
        try {
            ...
            logger.error('CodeParrot connection test failed:', error);
