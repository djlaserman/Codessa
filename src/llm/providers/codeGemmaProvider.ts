 ...
 */
export class CodeGemmaProvider extends BaseLLMProvider {
    readonly providerId = 'codegemma';
    readonly displayName = 'CodeGemma';
    readonly description = 'Code-specialized LLMs from Google';
    readonly website = 'https://huggingface.co/google/codegemma';
    ...
    readonly defaultModel = 'google/codegemma-7b-it';
    ...
    constructor(context?: vscode.ExtensionContext) {
        ...
        vscode.workspace.onDidChangeConfiguration(e => {
            if (e.affectsConfiguration('codessa.llm.providers')) {
                logger.info("CodeGemma configuration changed, re-initializing client.");
    ...
    private initializeClient() {
        ...
        if (!apiKey) {
            logger.warn('API key not set for CodeGemma provider.');
            this.client = null;
            return;
        }
        ...
        try {
            ...
            logger.info('CodeGemma client initialized successfully.');
        } catch (error) {
            logger.error('Failed to initialize CodeGemma client:', error);
            this.client = null;
        }
    }
    ...
    async generate(
        ...
        try {
            ...
            logger.debug(`Sending request to CodeGemma model ${modelId}`);
            ...
            if (cancellationToken) {
                if (typeof AbortController !== 'undefined') {
                    ...
                    cancellationToken.onCancellationRequested(() => {
                        logger.info("CodeGemma request cancelled by user");
            ...
            logger.error('CodeGemma generate error:', error);
    ...
    public async testConnection(modelId: string): Promise<{success: boolean, message: string}> {
        ...
        try {
            ...
            logger.error('CodeGemma connection test failed:', error);
