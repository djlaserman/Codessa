...
(() => {
    ...
    function addMessageToUI(message, isInitial = false) {
        ...
        if (document.getElementById(fullMessage.id)) {
            return;
        }
        const messageWrapper = document.createElement('div');
        messageWrapper.className = `message ${fullMessage.role}`;
        messageWrapper.id = fullMessage.id;
        ...
        avatar.innerHTML = getAvatarIcon(fullMessage.role);
        if (fullMessage.role !== 'system') {
        ...
        try {
            contentElement.innerHTML = marked.parse(fullMessage.content || '');
        }
        catch (e) {
            console.error("Markdown parsing error:", e);
            contentElement.textContent = fullMessage.content || '';
        }
        bubble.appendChild(contentElement);
        const timestampElement = document.createElement('div');
        timestampElement.className = 'timestamp';
        timestampElement.textContent = formatTimestamp(fullMessage.timestamp);
    ...
    function enhanceCodeBlock(preElement) {
        ...
        if (!codeElement || preElement.querySelector('.code-block-header'))
            return;
        ...
        copyButton.addEventListener('click', () => {
            ...
            navigator.clipboard.writeText(codeToCopy).then(() => {
                ...
                if (statusSpan) {
                    ...
                    setTimeout(() => { if (statusSpan)
                        statusSpan.textContent = 'Copy'; }, 1500);
                }
            });
        });
    ...
    function handleInputCopy() {
        if (isProcessing || !messageInput.value)
            return;
        messageInput.select();
        ...
        if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(messageInput.value).then(() => {
                ...
                try {
                    ...
                    console.error('execCommand cut failed: ', e);
                }
            });
        }
        else {
