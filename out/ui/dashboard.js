...
const providerSettingsPanel_1 = require("./providerSettingsPanel");
/**
...
class DashboardPanel {
    ...
    static createOrShow(extensionUri, context) {
        logger_1.logger.info('Creating or showing dashboard panel...');
        ...
        logger_1.logger.info('Creating new dashboard panel...');
        try {
            ...
            logger_1.logger.info('Dashboard panel created successfully.');
            ...
            logger_1.logger.error('Error creating dashboard panel:', error);
            ...
            switch (message.command) {
                ...
                case 'openProviderSettings':
                    if (message.providerId) {
                        ...
                        const provider = llmService_1.llmService.getProvider(message.providerId);
                        if (provider) {
                            providerSettingsPanel_1.ProviderSettingsPanel.createOrShow(this._extensionUri, message.providerId, this._context);
                        }
                        else {
     ...
     */
    async _update() {
        logger_1.logger.info('Updating dashboard content...');
        try {
            const webview = this._panel.webview;
            this._panel.title = 'Codessa Dashboard';
            // Gather data for the dashboard
            logger_1.logger.info('Gathering dashboard data...');
            const dashboardData = await this._getDashboardData();
            logger_1.logger.info('Dashboard data gathered successfully.');
            // Update HTML content
            logger_1.logger.info('Generating webview HTML content...');
            webview.html = this._getWebviewContent(webview, dashboardData);
            logger_1.logger.info('Dashboard content updated successfully.');
        }
        catch (error) {
            logger_1.logger.error('Error updating dashboard content:', error);
    ...
    async _getDashboardData() {
        try {
            ...
            const providerStatus = providers.map(id => {
                const provider = llmService_1.llmService.getProvider(id);
                return {
                    id,
            ...
            // Get agents
            const agents = agentManager_1.agentManager.getAllAgents().map(agent => ({
                id: agent.id,
                name: agent.name,
            ...
            logger_1.logger.error('Error gathering dashboard data:', error);
            // Return minimal data
            return {
    ...
    _getWebviewContent(webview, data) {
        ...
        try {
            ...
            logger_1.logger.error('Error generating webview HTML content:', error);
            return `<!DOCTYPE html>
            <html lang="en">
            <head>
                <meta charset="UTF-8">
...
DashboardPanel.viewType = 'codessaDashboard';
