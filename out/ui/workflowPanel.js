...
function convertToUIWorkflow(workflow) {
    ...
    return {
        ...
        version: isGraphDefinition ? workflow.version || '1.0.0' : workflow.version,
        steps: isGraphDefinition ? workflow.nodes : workflow.steps,
...
class WorkflowPanel {
    constructor(panel, extensionUri) {
        ...
        this._panel.webview.onDidReceiveMessage(async (message) => {
            ...
            if (message.command === 'getAvailableToolsWithActions') {
                try {
                    const { toolRegistry } = await Promise.resolve().then(() => __importStar(require('../tools/toolRegistry')));
            ...
            try {
                logger_1.logger.info(`Received message from webview: ${message.command}`);
    ...
    async _handleSaveWorkflow(workflow) {
        ...
        logger_1.logger.info(`Saved workflow: ${workflow.name} (${workflow.steps.length} steps)`);
    }
    async _update() {
    ...
    async _getHtmlForWebview(webview) {
        ...
        try {
            ...
            const allWorkflows = (0, workflows_1.getAllWorkflows)();
            ...
            logger_1.logger.info(`Found ${workflowsData.length} original workflows and ${filteredLangGraphWorkflows.length} LangGraph workflows`);
            ...
            if (filteredLangGraphWorkflows.length > 0) {
                ...
                const workflowDetails = (0, workflows_1.getWorkflowById)(sampleWorkflowId);
                if (workflowDetails) {
                    // Access the workflow object correctly based on its type
                    const workflowName = workflowDetails.workflow.name;
                    logger_1.logger.debug(`Sample workflow details: ${workflowName}`);
                    try {
                        // Create an instance but don't execute it
                        const instance = (0, workflows_1.createWorkflowInstance)(sampleWorkflowId);
                        // Check instance type and access properties accordingly
                        if ('getDefinition' in instance) {
                            // It's a Workflow instance
                            logger_1.logger.debug(`Created workflow instance for: ${instance.getDefinition().name}`);
                        }
                        else {
                            // It's a LangGraph instance
                            logger_1.logger.debug(`Created LangGraph instance for workflow: ${workflowName}`);
                        }
                    }
                    catch (error) {
                        logger_1.logger.error(`Error creating workflow instance: ${error}`);
                    }
                }
            }
...
<body>
    ...
    <script nonce="${nonce}" src="${scriptUri}"></script>
            ...
            logger_1.logger.error('Error generating workflow panel HTML:', error);
            return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
WorkflowPanel._loadedWorkflows = [];
WorkflowPanel.viewType = 'codessa.workflowPanel';
