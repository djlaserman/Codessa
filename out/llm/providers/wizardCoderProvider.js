...
exports.WizardCoderProvider = void 0;
 ...
 */
class WizardCoderProvider extends baseLLMProvider_1.BaseLLMProvider {
    constructor(context) {
        super(context);
        this.providerId = 'wizardcoder';
        this.displayName = 'WizardCoder';
        this.description = 'Code-specialized LLMs fine-tuned from Code Llama';
        this.website = 'https://huggingface.co/WizardLM';
        ...
        this.defaultModel = 'WizardLM/WizardCoder-Python-34B-V1.0';
        this.client = null;
        this.initializeClient();
        // Listen for configuration changes
        vscode.workspace.onDidChangeConfiguration(e => {
            if (e.affectsConfiguration('codessa.llm.providers')) {
                logger_1.logger.info("WizardCoder configuration changed, re-initializing client.");
    ...
    initializeClient() {
        ...
        if (!apiKey) {
            logger_1.logger.warn('API key not set for WizardCoder provider.');
            this.client = null;
            return;
        }
        try {
            ...
            logger_1.logger.info('WizardCoder client initialized successfully.');
        }
        catch (error) {
            logger_1.logger.error('Failed to initialize WizardCoder client:', error);
            this.client = null;
        }
    }
    isConfigured() {
    ...
    async generate(params, cancellationToken, tools) {
        ...
        try {
            ...
            logger_1.logger.debug(`Sending request to WizardCoder model ${modelId}`);
            ...
            if (cancellationToken) {
                if (typeof AbortController !== 'undefined') {
                    ...
                    cancellationToken.onCancellationRequested(() => {
                        logger_1.logger.info("WizardCoder request cancelled by user");
            ...
            logger_1.logger.error('WizardCoder generate error:', error);
    ...
    async testConnection(modelId) {
        ...
        try {
            ...
            logger_1.logger.error('WizardCoder connection test failed:', error);
...
exports.WizardCoderProvider = WizardCoderProvider;
//# sourceMappingURL=wizardCoderProvider.js.map