...
class YiCodeProvider extends baseLLMProvider_1.BaseLLMProvider {
    constructor(context) {
        super(context);
        this.providerId = 'yicode';
        this.displayName = 'Yi-Code';
        this.description = 'Powerful code model from 01.ai';
        this.website = 'https://01.ai/';
        ...
        this.defaultModel = '01-ai/Yi-34B-Code';
        this.client = null;
        this.initializeClient();
        // Listen for configuration changes
        vscode.workspace.onDidChangeConfiguration(e => {
            if (e.affectsConfiguration('codessa.llm.providers')) {
                logger_1.logger.info("Yi-Code configuration changed, re-initializing client.");
    ...
    initializeClient() {
        ...
        if (!apiKey) {
            logger_1.logger.warn('API key not set for Yi-Code provider.');
            this.client = null;
            return;
        }
        try {
            ...
            logger_1.logger.info('Yi-Code client initialized successfully.');
        }
        catch (error) {
            logger_1.logger.error('Failed to initialize Yi-Code client:', error);
            this.client = null;
        }
    }
    isConfigured() {
    ...
    async generate(params, cancellationToken, tools) {
        ...
        try {
            ...
            logger_1.logger.debug(`Sending request to Yi-Code model ${modelId}`);
            ...
            if (cancellationToken) {
                if (typeof AbortController !== 'undefined') {
                    ...
                    cancellationToken.onCancellationRequested(() => {
                        logger_1.logger.info("Yi-Code request cancelled by user");
            ...
            logger_1.logger.error('Yi-Code generate error:', error);
    ...
    async testConnection(modelId) {
        ...
        try {
            ...
            logger_1.logger.error('Yi-Code connection test failed:', error);
