...
exports.CodeGemmaProvider = void 0;
...
/**
 * Provider for CodeGemma models from Google
 *
 * CodeGemma is a family of code-specialized LLMs from Google
 * It can be used via Hugging Face or self-hosted
 */
class CodeGemmaProvider extends baseLLMProvider_1.BaseLLMProvider {
    constructor(context) {
        super(context);
        this.providerId = 'codegemma';
        this.displayName = 'CodeGemma';
        this.description = 'Code-specialized LLMs from Google';
        this.website = 'https://huggingface.co/google/codegemma';
        ...
        this.defaultModel = 'google/codegemma-7b-it';
        this.client = null;
        this.initializeClient();
        // Listen for configuration changes
        vscode.workspace.onDidChangeConfiguration(e => {
            if (e.affectsConfiguration('codessa.llm.providers')) {
                logger_1.logger.info("CodeGemma configuration changed, re-initializing client.");
    ...
    initializeClient() {
        ...
        if (!apiKey) {
            logger_1.logger.warn('API key not set for CodeGemma provider.');
            this.client = null;
            return;
        }
        try {
            ...
            logger_1.logger.info('CodeGemma client initialized successfully.');
        }
        catch (error) {
            logger_1.logger.error('Failed to initialize CodeGemma client:', error);
            this.client = null;
        }
    }
    isConfigured() {
    ...
    /**
     * Generate text using CodeGemma models
     */
    async generate(params, cancellationToken, tools) {
        if (!this.client) {
            return { content: '', error: 'CodeGemma provider not configured (API key missing?)' };
        }
        try {
            ...
            // Prepare the prompt
            let prompt = '';
            // CodeGemma uses a specific format for chat
            if (params.systemPrompt) {
                prompt += `<start_of_turn>system\n${params.systemPrompt}<end_of_turn>\n\n`;
            }
            ...
            const requestData = {
                ...
                parameters: {
                    ...
                    stop: params.stopSequences || ["<start_of_turn>", "<end_of_turn>"]
                }
            };
            logger_1.logger.debug(`Sending request to CodeGemma model ${modelId}`);
            ...
            if (cancellationToken) {
                if (typeof AbortController !== 'undefined') {
                    ...
                    cancellationToken.onCancellationRequested(() => {
                        logger_1.logger.info("CodeGemma request cancelled by user");
            ...
            logger_1.logger.error('CodeGemma generate error:', error);
            let errorMessage = 'Failed to call CodeGemma API.';
            if (error.response) {
                errorMessage = `CodeGemma API Error (${error.response.status}): ${error.response.data?.error || error.message}`;
            }
    ...
    /**
     * List available CodeGemma models
     */
    async listModels() {
        // CodeGemma models are fixed, so we return a predefined list
        return [
            {
                id: 'google/codegemma-7b-it',
                name: 'CodeGemma 7B Instruct',
                description: 'Instruction-tuned CodeGemma model (7B parameters)',
                contextWindow: 8192,
            ...
            },
            {
                ...
                name: 'CodeGemma 7B',
                description: 'Base CodeGemma model (7B parameters)',
                contextWindow: 8192,
                pricingInfo: 'Free with Hugging Face API'
            },
            {
                id: 'google/codegemma-2b-it',
                name: 'CodeGemma 2B Instruct',
                description: 'Instruction-tuned CodeGemma model (2B parameters)',
                contextWindow: 8192,
                pricingInfo: 'Free with Hugging Face API'
            },
            {
                ...
                name: 'CodeGemma 2B',
                description: 'Base CodeGemma model (2B parameters)',
                contextWindow: 8192,
                pricingInfo: 'Free with Hugging Face API'
            }
        ];
    ...
    /**
     * Test connection to CodeGemma
     */
    async testConnection(modelId) {
        if (!this.client) {
            return {
                success: false,
                message: 'CodeGemma client not initialized. Please check your API key.'
            };
        }
        try {
            ...
            logger_1.logger.error('CodeGemma connection test failed:', error);
    ...
    getConfigurationFields() {
        return [
            ...
            },
            {
                id: 'defaultModel',
                name: 'Default Model',
                description: 'The default CodeGemma model to use',
                required: false,
                type: 'select',
                options: [
...
exports.CodeGemmaProvider = CodeGemmaProvider;
//# sourceMappingURL=codeGemmaProvider.js.map